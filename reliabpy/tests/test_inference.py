...
class TestALL(unittest.TestCase):
    def setUp(self):
        ...
        self.dbn.initialize(T, discretizations, b0)
        while self.dbn.t <= lifetime:
            self.dbn.predict()

            if self.dbn.t in inspection_years:
                self.dbn.update(model='PoD', parameters={'quality': 'bad'})

        # MCS
        self.mcs = MonteCarloSimulation()
        self.mcs.force_detection, self.mcs.force_notdetection = False, True
        self.mcs.initialize(a_0, function, a_crit)
